...
3. One accent color (green) in ALL site UI. Exception: illustration artwork (hero scene, future Lottie vignettes) uses the Bauhaus scene palette: scene-red, scene-blue, scene-yellow, scene-pink, indigo + ink, on cream. Deliberately NOT the Google primaries. These colors never leak into buttons, links, chips or body text. Mapping in the hero orrery: planets = red (the character, innermost), yellow, blue (+ indigo moon), pink (+ indigo ring); sun, orbits, stars & comet = ink; sun glow & rim = scene-yellow; un-ignited planets = muted. Clips: simple = blue, light = yellow (label in ink for contrast), deep = indigo; playhead & diamonds = ink.
...
- Hero scene "The Timeline" is DONE in code: `src/components/TimelineScene.astro` (Canvas 2D stage + SVG timeline + vanilla JS, zero deps). The stage is an orrery drawn EDGE-ON: all orbits collapse into one dashed line. Over "simple" it is a grey line drawing; over "light" the planets ignite inner-to-outer in the scene palette and the sun glows; entering "deep" the system snaps into a tilted 3D view (underdamped spring on orbit squash, sharp open / stiff close) revealing full orbits, depth-sorted passes behind the sun, faint stars, a moon, a ring and a rare comet. Behavior contract: pointer X scrubs the playhead (lerp); touch scrubs only after 8px horizontal intent (vertical stays page scroll); idle loop 9s sweep + 2.5s hold on the 3D view + 0.6s rewind; cooldown 1.5–2s before idle resumes from the current position; IntersectionObserver pauses offscreen; prefers-reduced-motion shows the static deep frame. Every visual parameter is a pure function of playhead position plus a free-running clock — scrubbing is fully bidirectional. A Rive rebuild may replace the innards later — keep the same behavior contract. Color key lives on the Figma page "Rive — The Timeline (storyboard)".
